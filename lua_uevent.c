...
static int close_connection(uevent_conn_t* conn) {
...
{
	if (!L || !data->conn || data->conn->closing || data->conn->closed) {
		return;
	}
...
static int uevent_run(lua_State *L)
{
	...
	if (conn->closing){
		return uevent_failmsg(L, "UEVENT connection closing", "connection is closing");
	}
	...
	if (node != NULL) {
		while (node && !conn->closing && !conn->closed) {
...
static int destroy_connection (uevent_conn_t *conn);
static void* connection_proc(void* arg)
{
    ...
    while (1) {
        if (conn->closing || conn->closed || conn->sock < 0) {
            break;
        }
...
static int create_netlink(lua_State *L, uevent_conn_t* conn, unsigned int groups)
{
	...
	if (ret < 0) {
		close(sockfd);
		return uevent_failmsg(L, "setsockopt RECVTIMEO error ", strerror(errno));
	}
...
static int destroy_connection(uevent_conn_t *conn) {
	...
	// Mark connection as closed
	conn->closed = 1;
	luaL_unref (conn->state, LUA_REGISTRYINDEX, conn->env);
	conn->env = LUA_NOREF;
	...
	if (conn->callback) {
		luaL_unref (conn->state, LUA_REGISTRYINDEX, conn->callback->callback);
		free(conn->callback);
		conn->callback = NULL;
	}
	conn->state = NULL;
